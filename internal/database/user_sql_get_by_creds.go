...
func GetUserByCreds(email string, unhashedPwd string) (*models.User, error) {
	...
	selectClause = `
		SELECT
			...
			phone, preferred_city, preferred_locale, preferred_sport, preferred_theme,
	...
	if err := GetClient().QueryRow(selectClause, email).Scan(
		...
		&user.Phone, &user.PreferredCity, &user.PreferredLocale, &user.PreferredSport, &user.PreferredTheme,
